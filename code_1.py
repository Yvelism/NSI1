...
class perso:
    ...
    def faiblesse(self):
        if self.pv_courant<0.5*self.pv_init:
            nf=self.force-0.05*self.force
            self.set_force(nf)
        if self.pv_courant<0.25*self.pv_init:
            nf=self.force-0.1*self.force
            self.set_force(nf)
...
#création des persos
pit=perso("Pit","images/pit.png", 250,250, 80, 80)
mario=perso("Mario","images/mario.png", 200, 200, 50, 50)     
ike = perso("Ike","images/ike.png",200, 200, 40, 40)
